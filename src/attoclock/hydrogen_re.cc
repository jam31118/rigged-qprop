...
int main(int argc, char **argv) {
  ...
  const double grid_size=para_prop.getDouble("imag-width")+para_tsurff.getDouble("R-tsurff");
  cout << "N_R: " << long(grid_size/delta_r) << endl;
  ...
  // A_x and A_y are non zero in this example
  vecpot vecpot_x(omega, para_prop.getDouble("num-cycles"), E_0, 0.0);
  vecpot vecpot_y(omega, para_prop.getDouble("num-cycles"), E_0, 0.5*M_PI);
  vecpot vecpot_z(omega, 1.0, 0.0, 0.0);
  const double U_p     = vecpot_x.get_Up();
  const double gamma_K = sqrt(I_p / 2.0 / U_p);     
  const double pulse_duration=vecpot_x.get_duration();
  ...
  string str_fname_vpot_xy=common_prefix+string("-vpot_xy.dat");
  FILE* file_vpot_xy = fopen_with_check(str_fname_vpot_xy, "w");
  for (long ts=0; ts<lno_of_ts; ts++) {
    const double time=real_timestep*double(ts);
    if (ts%ldumpwidth==0)
      fprintf(file_vpot_xy, "%15.10le %15.10le %15.10le\n", time, vecpot_x(time, me), vecpot_y(time, me));
  };
  fclose(file_vpot_xy);
  ...
  for (long ts=0; ts<lno_of_ts; ts++) {
    ...
    if (ts%ldumpwidth==0) {
      ...
      N = wf.norm(g_prop);
      polplus=wf.expect_cycl_pol_plus(g_prop);
      polminus=wf.expect_cycl_pol_minus(g_prop);
      x_expect=real(0.5*(polplus+polminus));
      y_expect=real(-0.5*imagi*(polplus-polminus));

      fprintf(file_obser,"%10.5le %20.15le %20.15le %20.15le %20.15le %20.15le\n",time,E_tot,real(conj(P)*P),N,x_expect,y_expect);
    };
    //
    // propagate one step forward in (real) time.
    // my_m_quantum_num is ignored for mode 44
