...
int main(int argc, char **argv) {
  ...
  string str_fname_wf_ini=string("./hydrogen_im-") 
    + to_string(my_m_quantum_num) + string("-wf_fin.dat");
...
//  wf.regrid(g_prop, g_load, wf_load);
  ...
  if (start_time_index == 0) { start_wf_file_name = string("ini-wf.bin"); }
  else if (start_time_index > 0) { start_wf_file_name = string("current-wf.bin"); }
  else { 
  ...
  if (!start_wf_file.is_open()) { 
    ...
    return 1; }
  wf.load_from_binary(start_wf_file);
  start_wf_file.close();
  fclose(file_wf_ini);
